                ...
                return (
                  <tr key={o.id} className={`${isEvenBg} hover:bg-indigo-100/60 transition-colors duration-150`}>
                    ...
                    {COLUMNS.map((col) => {
                      ...
                      if (col.key === "_index") return <td key={col.key} data-frozen="1" className={`${CELL_CLASS} text-center text-gray-400 text-xs font-mono sticky ${isEvenBg} z-10 ${lastFrozen ? "shadow-[2px_0_6px_rgba(0,0,0,0.1)]" : ""}`} style={{position:"sticky",left:38,zIndex:10,transform:"translateZ(0)",boxShadow:"1px 0 0 0 "+(isEvenBg==="bg-white"?"#fff":"#eef2ff")}}>{rowIdx + 1}</td>;
